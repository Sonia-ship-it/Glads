...
describe('Bookings Integration Tests', () => {
    ...
    describe('POST /bookings/check-availability', () => {
        it('should check room availability', async () => {
            const response = await request(app.getHttpServer())
                ...
                .expect(201);
            const availability = typeof response.body === 'boolean'
                ? response.body
    ...
    describe('GET /bookings', () => {
        it('should return list of bookings for staff', async () => {
            ...
            expect(response.body.some((booking) => booking.id === testBookingId)).toBe(true);
        });
...
//# sourceMappingURL=bookings.integration-spec.js.map