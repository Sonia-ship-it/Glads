...
describe('Rooms Integration Tests', () => {
  ...
  describe('GET /rooms', () => {
    it('should return list of active rooms', async () => {
      ...
      expect(response.body.some((room: any) => room.id === testRoomId)).toBe(true);
    });
    ...
    it('should search available rooms', async () => {
      const response = await request(app.getHttpServer())
        .get(
          `/rooms/search?branchId=${testBranchId}&checkInDate=2026-03-10T14:00:00Z&checkOutDate=2026-03-12T11:00:00Z&numberOfGuests=2`,
        )
        .expect(200);
